...
public class PanelCoverage extends PanelAbstract {
    ...
    private boolean isCoverageDrawable(int i) {
        return (m_covS != null && m_covS.size() > 0 && m_covT != null && m_covT.size() > 0 && m_covTP != null
                        && m_covTP.size() > 0 && m_covA != null && m_covA.size() > 0 && m_covS.size() > i
                        && m_covT.size() > i && m_covTP.size() > i && m_covA.size() > i) ? true : false;
    }
