...
/**
 * Panel for displaying the efficiency graph for Transition Coverage History of
 * RandomTester, GreedyTester, LookaheadTester, QuickTester
 */
public class PanelEfficiencyGraph extends PanelAbstract {
  ...
  protected void paintComponent(Graphics g) {
    ...
    final int nInternalSpaceY = (AXIS_HEIGHT - ARROW_LENGTH)
        / (PanelEfficiencyGraph.m_nScaleNumber + 2);
    ...
    final int internalSpaceX = (AXIS_WIDTH - ARROW_LENGTH)
        / (PanelEfficiencyGraph.m_nScaleNumber + 1);
  ...
  private boolean isCoverageDrawable(int i) {
    return (mRandomTesterCoverage != null && mRandomTesterCoverage.size() > 0 && mGreedyTesterCoverage != null
        && mGreedyTesterCoverage.size() > 0 && mLookaheadTesterCoverage != null && mLookaheadTesterCoverage.size() > 0
        && mQuickTesterCoverage != null && mQuickTesterCoverage.size() > 0 && mRandomTesterCoverage.size() > i
        && mGreedyTesterCoverage.size() > i && mLookaheadTesterCoverage.size() > i && mQuickTesterCoverage.size() > i)
        ? true
        : false;
  }
