...
public class Indentation {
    ...
    protected static void shrink() {
        m_strIndentation = m_strIndentation.substring(0, m_strIndentation.length() - INDENTATION.length());
    }

    /**
     * This adds the current indentation prefix to str. As a side effect, it also adjusts the current indentation level
     * up by one if str contains a '{' character, or down by one if it contains a '}' character.
     * 
     * @param str
