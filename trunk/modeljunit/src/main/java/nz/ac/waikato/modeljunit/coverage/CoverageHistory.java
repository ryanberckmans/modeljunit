...
/**
 * A wrapper class that adds history recording to any CoverageMetric. This is useful for drawing graphs of how the
 * coverage changes over time etc. For example, new CoverageHistory(new ActionCoverage(), 10) behaves exactly the same
 * as the ActionCoverage metric, but also records a snapshot of its coverage level each 10 calls to doneTransition(...).
 * <p>
 * Note that you must add this CoverageHistory wrapper as the listener to the model, rather than adding the underlying
 * metric. If you add both, the metrics will be recorded twice as fast, which is probably not what you want!
 * </p>
 * 
 * @author marku
 */
public class CoverageHistory extends AbstractListener implements CoverageMetric {
    ...
    /**
     * Create a history version of the coverage metric. It records the value of metric.getCoverage() each interval calls
     * to doneTransition.
     * 
     * @param metric
     *            The coverage metric to add history to.
     * @param interval
    ...
    public CoverageHistory(/*@non_null@*/CoverageMetric metric, int interval) {
        ...
        if (metric.getModel() != null) {
            metric.getModel().printWarning("You should add CoverageHistory metric, rather than " + metric.getName());
        }
    }
    ...
    /**
     * This tells the underlying metric about the model, even though the model will not be sending events directly to
     * that metric (the events come through this wrapper).
     */
    @Override
