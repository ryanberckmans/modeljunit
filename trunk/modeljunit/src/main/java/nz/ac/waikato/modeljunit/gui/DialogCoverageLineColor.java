...
public class DialogCoverageLineColor extends JDialog {
    ...
    public static final Color[] LINE_COVERAG_COLOR = { Color.BLACK, Color.RED, Color.GREEN, Color.BLUE, Color.GRAY,
                    Color.YELLOW, Color.ORANGE, Color.CYAN };

    private static final String[] STR_COVERAGE_NAME = { "State", "Transition", "Transition pair", "Action" };
    ...
    private void createComboBoxes() {
        for (int i = 0; i < Parameter.NUM_COVERAGE; i++) {
            ComboBoxItemRender r = new ComboBoxItemRender(getAvailableColorList(STR_COVERAGE_NAME[i]));
            r.setPreferredSize(new Dimension(20, 10));
            m_modelComboBox[i] = new ComboBoxModel(getAvailableColorList(STR_COVERAGE_NAME[i]), i);
    ...
    /**
     * Get color from parameter class "State" State coverage line color "Transition" Transition coverage line color
     * "Transition Pair" Transition pair coverage line color "Action" Action coverage line color
     * */
    ...
    /**
     * Two coverage line color are excluding. This function is for calculate which color is available for particular
     * coverage.
     * */
    private Color[] getAvailableColorList(String coverage) {
        // Calculate how many colors available
        Color[] availableColor = new Color[LINE_COVERAG_COLOR.length - Parameter.NUM_COVERAGE + 1];
        ...
        for (int i = 0; i < m_colorLine.length; i++) {
            if (m_colorLine[i].m_strCoverage == null) {
                ...
                nAvailableColorIdx++;
            } else if (m_colorLine[i].m_strCoverage != null && m_colorLine[i].m_strCoverage.equals(coverage)) {
                availableColor[nAvailableColorIdx] = m_colorLine[i].m_color;
    ...
    private class ComboBoxModel extends DefaultComboBoxModel {
        ...
        /**
         * color: the data that the combobox owns id: id of the combobox box 0.State coverage 1.Transition coverage
         * 2.Transition pair coverage 3.action coverage
         */
        ...
        public int getIndexOf(Object o) {
            ...
            for (int i = 0; i < m_color.length; i++) {
                if (m_color[i].getRed() == ((Color) o).getRed() && m_color[i].getBlue() == ((Color) o).getBlue()
